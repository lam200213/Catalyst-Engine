      ...
      "definitions": {
        "VCPChartData": {
          ...
          "properties": {
            ...
            "vcpLines": { 
              ...
              "type": "array", 
              "items": { "type": "object" },
              "description": "Deprecated in favor of vcpContractions but kept for backward compatibility."
            },
            "vcpContractions": {
              "title": "VCP Contractions",
              "type": ["array", "null"],
              "items": { "$ref": "#/definitions/VCPContractionItem" },
              "description": "Granular list of individual contractions for visualization."
            },
            "pivotPrice": {
              "title": "Pivot Price",
              "type": ["number", "null"],
              "description": "Identified pivot price for the pattern."
            },
            "vcp_pass": {
              "title": "VCP Pass",
              "type": ["boolean", "null"],
              "description": "Pass/Fail status specific to the chart context."
            },
            "rejection_reason": {
              "title": "Rejection Reason",
              "type": ["string", "null"],
              "description": "Concise reason for VCP failure."
            },
            "buyPoints": { "title": "Buypoints", "type": "array", "items": { "type": "object" }},
            "sellPoints": { "title": "Sellpoints", "type": "array", "items": { "type": "object" }},
            "ma20": { "title": "Ma20", "type": "array", "items": { "type": "object" }},
            "ma50": { "title": "Ma50", "type": "array", "items": { "type": "object" }},
            "ma150": { "title": "Ma150", "type": "array", "items": { "type": "object" }},
            "ma200": { "title": "Ma200", "type": "array", "items": { "type": "object" }},
            "historicalData": { "title": "Historicaldata", "type": "array", "items": { "type": "object" }}
          },
          "required": ["detected", "vcpLines", "buyPoints", "sellPoints", "ma20", "ma50", "ma150", "ma200", "historicalData"]
        },
        "VCPContractionItem": {
          "title": "VCPContractionItem",
          "type": "object",
          "description": "Represents a single contraction within a detected VCP pattern.",
          "properties": {
            "start_date": {
              "title": "Start Date",
              "type": "string",
              "description": "Date of the contraction peak."
            },
            "start_price": {
              "title": "Start Price",
              "type": "number",
              "description": "Price at the contraction peak."
            },
            "end_date": {
              "title": "End Date",
              "type": "string",
              "description": "Date of the contraction trough."
            },
            "end_price": {
              "title": "End Price",
              "type": "number",
              "description": "Price at the contraction trough."
            },
            "depth_percent": {
              "title": "Depth Percent",
              "type": "number",
              "description": "Percentage depth of this contraction (0.0 to 1.0)."
            }
          },
          "required": ["start_date", "start_price", "end_date", "end_price", "depth_percent"]
        },
        "VCPDetailCheck": {
        ...
        "VCPDetails": {
          ...
          "properties": {
            ...
            "volume_validation": { 
              "$ref": "#/definitions/VCPDetailCheck",
    ...
    ```json
    {
      ...
      "properties": {
        "passing_candidates": {
          ...
          "type": "array",
          "items": { "$ref": "#/definitions/LeadershipProfileForBatch" }
        },
        "unique_industries_count": {
          "title": "Unique Industries Count",
          "type": "integer"
        },
        "metadata": { "$ref": "#/definitions/LeadershipProfileBatchMetadata" }
      },
      "required": ["passing_candidates", "unique_industries_count", "metadata"],
      "definitions": {
        "LeadershipSummary": {
          "title": "LeadershipSummary",
          "type": "object",
        ...
        "LeadershipProfileForBatch": {
          ...
          "required": ["ticker", "passes", "leadership_summary", "profile_details"]
        },
        "LeadershipProfileBatchMetadata": {
          "title": "LeadershipProfileBatchMetadata",
          "type": "object",
          "properties": {
            "total_processed": { "title": "Total Processed", "type": "integer" },
            "total_passed": { "title": "Total Passed", "type": "integer" },
            "execution_time": { "title": "Execution Time", "type": "number" }
          },
          "required": ["total_processed", "total_passed", "execution_time"]
        }
      }
    }
    ```
...
-   **Consumers:** MongoDB `screening_jobs` collection, `api-gateway`/`frontend-app`
...
-   **Description:** This contract aggregates three key pieces of information: a high-level `market_overview` (including the market stage, correction depth, and breadth indicators), a detailed breakdown of the current market `leaders_by_industry`, and (optionally) a map of per-index VCP analysis results in `indices_analysis`. This structure allows the UI to build the entire Market page from a single API call and, when present, render richer index-level charts.
-   **JSON Schema:**
    ```json
    {
      ...
      "properties": {
        ...
        "indices_analysis": {
          ...
          "description": "Optional map of index symbol -> VCPAnalysisSingle (see contract 7 VCPAnalysis for full schema). Enables chart rendering for major indices.",
          "type": ["object", "null"],
...
-   **JSON Schema:**
    ```json
    {
      ...
      "required": ["industry", "breadth_count"]
    }
    ```
...
-   **Consumers:** `frontend-app`, all internal service consumers
...
-   **Consumers:** `monitoring-service` route handlers
...
-   **Notes:**
    ...
    - The `AnalyzeFreshnessBatchItem` model uses `fresh` with `alias="passes_freshness_check"` so downstream services can work with a consistent boolean flag while still accepting the existing JSON shape.
    - The additional fields are optional and provide richer context for `monitoring-service` status logic but are not required for basic orchestration.
...
-   **Fields:**
    ...
    - `not_found`: Integer count of requested tickers not found
...
-   **Example Payload:**
    ```json
    {
      ...
      "removed_tickers": ["AAPL", "MSFT", "GOOGL"],
      "not_found_tickers": [],
      "removed": 3,
      "not_found": 0
    }
...
-   **JSON Schema:**
    ```json
    {
      ...
      "properties": {
        "items": {
          "type": "array",
          "items": { "$ref": "#/definitions/InternalBatchUpdateStatusItem" },
    ...
    ```json
    {
      ...
      "properties": {
        "items": {
          "title": "Items",
          "type": "array",
          "items": { "$ref": "#/definitions/InternalBatchArchiveFailedItem" }
        }
      },
      "required": ["items"],
      "definitions": {
...
-   **Producer:** `monitoring-service`
-   **Consumers:** `scheduler-service` (Celery task `refresh_watchlist_task`), job history, internal tools/automation
...
-   **JSON Schema:**
    ```json
    {
      ...
      "required": ["message", "updated_items", "archived_items", "failed_items"],
      "additionalProperties": false
    }
    ```
-   **Example Payload:**
...
Full response for `GET /monitor/watchlist`, including watchlist items and metadata.

-   **Producer:** `monitoring-service`
-   **Consumer(s):** `frontend-app` (via `api-gateway`)
-   **Pydantic Model:** `WatchlistListResponse`
-   **Description:** Returns a complete list of watchlist items for the default user, excluding portfolio tickers, along with a metadata summary. Each item contains all fields needed for the Phase 2 UI, including VCP pattern details, volume metrics, and derived status signals.
-   **JSON Schema:**
    ```json
    {
        ...
        "metadata": {
          ...
          "properties": {
            "count": {
              "title": "Count",
              "type": "integer",
              "description": "Total number of watchlist items returned"
            }
          },
          "required": ["count"]
        }
      ...
      "definitions": {
        "WatchlistItem": {
          ...
          "properties": {
            ...
            "status": {
              "title": "Status",
              "allOf": [{ "$ref": "#/definitions/WatchlistStatus" }],
              "description": "Derived status: Pending, Failed, Buy Ready, Buy Alert, Watch"
            },
            "date_added": {
              "title": "Date Added",
              "type": ["string", "null"],
            ...
            "is_favourite": {
              "title": "Is Favourite",
              "type": "boolean",
              "description": "Whether user marked as favourite"
            },
            "last_refresh_status": {
              "title": "Last Refresh Status",
              "allOf": [{ "$ref": "#/definitions/LastRefreshStatus" }],
          ...
          "required": ["ticker", "status", "is_favourite", "last_refresh_status", "is_leader"],
          "additionalProperties": false
        },
        "WatchlistStatus": {
          "title": "WatchlistStatus",
          "type": "string",
          "enum": ["Pending", "Failed", "Watch", "Buy Alert", "Buy Ready"],
        ...
        "LastRefreshStatus": {
          ...
          "type": "string",
          "enum": ["PENDING", "PASS", "FAIL", "UNKNOWN"],
          "description": "Health-check status enum"
        }
      }
    }
    ```
