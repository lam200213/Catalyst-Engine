...
class TestScheduler(unittest.TestCase):
    ...
    @patch('app.requests.post')
    @patch('app.requests.get')
    def test_job_continues_when_downstream_service_returns_malformed_json(self, mock_requests_get, mock_requests_post, mock_get_db_collection, mock_print):
        ...
        def get_side_effect(url, **kwargs):
            ...
            if 'ticker-service' in url:
                ...
                mock_resp.status_code = 200
                # This is the crucial part: the .json() method will raise an error
        ...
        # Mock the screening service to pass both tickers through
        mock_requests_post.return_value = MagicMock(
            status_code=200,
        ...
        # --- Assert ---
        # The job should complete successfully, even with the error
        self.assertEqual(response.status_code, 200)

        # Verify that only the valid ticker was saved to the database
        mock_results_collection.insert_many.assert_called_once()
        args, _ = mock_results_collection.insert_many.call_args
        self.assertEqual(len(args[0]), 1)
        self.assertEqual(args[0][0]['ticker'], 'PASS_TICKER')
